...
import WatchCarousel from "../components/Layout/WatchCarousel";
...
const WatchCard = ({ Watch, onAddToCart,onView }) => (
  <div className="relative bg-gray-900 text-white rounded-xl p-4 shadow-lg border border-gray-700 w-full flex flex-col">
        <div className="flex-1">
          ...
          />
          <h3 className="font-semibold text-xl line-clamp-1">{Watch.name}</h3>
          <p className="text-sm text-gray-300 mb-1">{Watch.ram} RAM | {Watch.storage} | {Watch.color}</p>
          <div className="flex items-center gap-2 mb-1">
            <p className="text-green-400 font-bold text-lg">₹{Watch.price}</p>
            <p className="line-through text-gray-500 text-sm">₹{Watch.originalPrice}</p>
          </div>
          <div className="text-yellow-400 mb-2">{renderStars(Watch.rating)}</div>
          <p className="text-sm text-gray-400 mb-4">Delivery: {Watch.delivery}</p>
        </div>
...
const Watchs = () => {
  ...
  const handleAddToCart = async (Watch) => {
    try {
      ...
      setAlertMsg(`${Watch.name} (${Watch.color}) added to cart`);
    } catch (error) {
      console.error("Add to cart failed:", error);
    }
  };
     const handleView = (Watch) => {
    ...
    <div className="min-h-screen p-6 relative">
      {alertMsg && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
          <div className="animate-slide-up bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg text-sm md:text-lg text-nowrap">
            {alertMsg}
          </div>
        </div>
      )}

      <h1 className="text-2xl font-bold mb-6 text-center">Watchs</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {ProductDetails.Watches.map((Watch, index) => (
          <WatchCard key={index} Watch={Watch} onAddToCart={handleAddToCart} onView={handleView}/>
        ))}
      </div>
    </div>
    {selectedProduct && (
